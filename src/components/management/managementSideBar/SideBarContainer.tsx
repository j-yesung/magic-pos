...
const SideBarContainer = ({ managementData }: { managementData?: StoreWithOrderInfo[] }) => {
  ...
  useEffect(() => {
    if (managementData) {
      const orderStoreFilterData = managementData?.[0].order_store.filter((x) => x.id === orderId)
      const orderNumberFilterData = managementData?.[0].order_number.filter((x) => x.id === orderId)
