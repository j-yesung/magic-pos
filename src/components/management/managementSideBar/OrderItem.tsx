...
const OrderItem = ({ orderData }: { orderData: Tables<'order_store'> }) => {
  ...
  const {
    refItemOrderList,
    refItemOrderListItem,
    refItemOrderListItemImage,
    refItemOrderListItemName,
    refItemOrderListItemRemainEa,
    refItemOrderListItemPrice,
    isListClick,
    setClickMenuList,
    setIsListClick,
  } = useManagementCssStore();
  ...
  useEffect(() => {
    ...
    if (
      refItemOrderListItemImage &&
      refItemOrderListItemImage.current &&
      refItemOrderListItemName &&
      refItemOrderListItemName.current &&
      refItemOrderListItemRemainEa &&
      refItemOrderListItemRemainEa.current &&
      refItemOrderListItemPrice &&
      refItemOrderListItemPrice.current
    ) {
      for (let i = 0; i < refItemOrderListItemImage.current.length; i++) {
        ...
        refItemOrderListItemImage.current?.[i].style?.setProperty(
          'visibility',
          `${isListClick ? 'visible' : 'hidden'}`,
        );
        refItemOrderListItemName.current?.[i].style?.setProperty('visibility', `${isListClick ? 'visible' : 'hidden'}`);
        refItemOrderListItemRemainEa.current?.[i].style?.setProperty(
          'visibility',
          `${isListClick ? 'visible' : 'hidden'}`,
        );
        refItemOrderListItemPrice.current?.[i].style?.setProperty(
          'visibility',
          `${isListClick ? 'visible' : 'hidden'}`,
        );
      }
      ...
      <ul className={styles['item-order-list']} ref={itemOrderListRef}>
        {menuList?.map((item, index) => {
          return (
            <li
              key={item.id}
              className={styles['item-order-list-item']}
              ref={el => (itemOrderListItemRef.current[index] = el as HTMLLIElement)}
            >
              <div>
                <span ref={el => (itemOrderListItemImageRef.current[index] = el as HTMLSpanElement)}>
                ...
                <span ref={el => (itemOrderListItemRemainEaRef.current[index] = el as HTMLSpanElement)}>
                  {item.remain_ea} 개
                </span>
              </div>
              <div>
                <span ref={el => (itemOrderListItemPriceRef.current[index] = el as HTMLSpanElement)}>
                  {convertNumberToWon(item.price)}
                </span>
              </div>
            </li>
          );
